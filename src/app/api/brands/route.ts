...
// GET - List brands (optimized for performance)
export async function GET(request: Request) {
  ...
  try {
    ...
    console.log(`[PERF] GET /api/brands: ${duration}ms`)
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (user.role !== 'tenant_owner' && user.role !== 'superadmin') {
      return NextResponse.json(
        { error: 'Only tenant owners can create brands' },
        { status: 403 }
      )
    }
    ...
    const duration = Date.now() - startTime
    console.log(`[PERF] POST /api/brands: ${duration}ms`)
